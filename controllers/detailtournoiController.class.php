...
class detailtournoiController extends template{
	...
	*/
	public function createFirstMatchsAction(){
		...
		$link = $this->checkBasicInputsAndRights()['link'];
		...
		if(!!$allTournTeams){
			...
			if($this->canMatchsBeCreated($matchedTournament)){
				...
				if( is_array($createdMatchs) && count($createdMatchs) > 0 ){
						...
						if($mm->create()){
							...
							if(!!$dbMatch)
								if(!$this->createMatchParticipantsOfMatch($dbMatch, $createdMatch->gtAllTeamsTournament()))
									$this->echoJSONerror("erreur: CFM_1", "Problème interne lors de la création des matchs d'ouverture");
							else{
								...
								$this->echoJSONerror("erreur: CFM_2", "Problème interne lors de la création des matchs d'ouverture");
							}
						}
						else
							$this->echoJSONerror("erreur: CFM_3", "Problème interne lors de la création des matchs d'ouverture");
					}
					...
					$this->echoJSONerror("erreur: CFM_4", "Problème interne lors de la création du match d'ouverture");
			}
		};
	}
	/*
		...
		if(!!$link && is_bool(strpos($link, 'null')) && $matchedTournament !== false && $this->getConnectedUser()->getId() == $matchedTournament->getIdUserCreator() && !is_numeric($matchedTournament->getIdWinningTeam())){
			...
			if(is_array($teamAndMatchArr)){
				...
				if($mm->setMatchWinner($m, $winnerTT)){
					...
					$this->echoJSONerror("erreur: DT_SW_4", "Impossible de définir l'équipe ".$matchedTournament->gtPublicTeamIdToPrint($winnerTT)." comme gagnante, si le problème persiste veuillez contacter un admin");
				exit;
			}
			else
	...
	*/
	private function getLastWinningTeams(tournament $t){
	...
	*/
	private function getFullyAlimentedTournament(tournament $t){
		// Recuperer tous les participants
		$rm = new registerManager();
		...
		if(!!$allMatchs){
			...
			$this->echoJSONerror("erreur: DT_GFAT_2", "aucun match n'a été créé pour ce tournoi !");			
	...
	*/
	private function isUserTournamentOwner($link){
		if(!!$link && is_bool(strpos($link, 'null'))) return false;

		$tm = new tournamentManager();
		$matchedTournament = $tm->getTournamentWithLink($link);
		// Si le tournoi existe bien en base
		if($matchedTournament !== false){
